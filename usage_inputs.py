...
app.layout = html.Div(
    [
        ...
        dash_blueprint.FormGroup(
            id="form-group",
            children=[
                dash_blueprint.NumericInput(
                    id="numeric-input-2",
                    key="numeric",
                    stepSize=0.01,
                    minorStepSize=0.01,
                ),
                dash_blueprint.InputGroup(
                    id="text-input-2", key="text", placeholder="Enter value here..."
                ),
            ],
        ),
...
@app.callback(Output("form-output", "children"), [Input("form-group", "childData")])
def form_input_changed(value):
    print("form changed - {}".format(value))
    return f"You have entered {value}"


@app.callback(Output("numeric-output", "children"), [Input("numeric-input", "value")])
def numeric_input_changed(value):
    print("button clicked! - {}".format(value))
    return f"You have entered {value}"


@app.callback(Output("text-output", "children"), [Input("text-input", "value")])
