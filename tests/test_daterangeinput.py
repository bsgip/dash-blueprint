...
def test_render_daterangeinput(dash_duo):
    ...
    start_day = 10
    end_day = 20
    start_day_input = get_elements_with_text(dash_duo, ".bp3-datepicker-day-wrapper", "10")[0]
    end_day_input = get_elements_with_text(dash_duo, ".bp3-datepicker-day-wrapper", "20")[0]
    ...
    assert date_inputs[0].get_attribute("value") == f"{year:04d}-{month:02d}-{start_day:02d}"
    # dash_duo.wait_for_text_to_equal(date_inputs[1], f"{year:04d}-{month:02d}-{end_day:02d}")
    assert date_inputs[1].get_attribute("value") == f"{year:04d}-{month:02d}-{end_day:02d}"
    

    dash_duo.wait_for_text_to_equal("#date-range-input-output", f"['{year:04d}-{month:02d}-{start_day:02d}', '{year:04d}-{month:02d}-{end_day:02d}'], True")
    dash_duo.wait_for_text_to_equal("#formgroup-valid", "True")
    dash_duo.wait_for_text_to_equal("#formgroup-output", f"{{'date-range-input': ['{year:04d}-{month:02d}-{start_day:02d}', '{year:04d}-{month:02d}-{end_day:02d}']}}")
    ...
    dash_duo.multiple_click(start_day_input, 1)
    dash_duo.wait_for_text_to_equal("#date-range-input-output", f"[None, '{year:04d}-{month:02d}-{end_day:02d}'], False")
    dash_duo.wait_for_text_to_equal("#formgroup-valid", "False")
