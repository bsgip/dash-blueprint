 ...
 */
 const RangeSelector = ({id, setProps, data, selected, margin, isDate, yLines, selectSingle, group, groupSize}) => {
    ...
    const setSelection = setProps ? (s) => setProps({selected: s}) : setSelectionRange;
    ...
    useEffect(() => {
      ...
      if (isDate) {
        ...
        xScale = d3.scaleTime()
          .domain([d3.min(data, d=> d[xAttr]), d3.max(data, d=> d[xAttr]) + axisExtra])
          .nice()
      ...
      const barWidth = xScale(groupedData[0][xAttr] + axisExtra) - xScale(groupedData[0][xAttr]);
    ...
    if (selection) {
        ...
        const brushLocation = [selection[0], selection[selection.length - 1] + axisExtra].map(xScale)
        svg.select("g.selector")
            .call(brush.move, brushLocation)
    }

      if (selectSingle) {
        svg.select("g.selector").call(g => g.select(".overlay")
      ...
      function brushed(event) {
        ...
        if (selection === null) {
          ...
          const [x0, x1] = selection.map(d => xScale.invert(d - barWidth / 2));
          ...
          const selected = pointsG.selectAll("rect").filter(d => x0 <= (d[xAttr]) && (d[xAttr]) < x2);
          const selectedX = selected.data().map(d => d.x);
          setSelectionRange(selectedX);
    ...
    return(
      <div id={id} ref={wrapperRef} style={{width: "100%", height: "100%", margin: "0 auto"}}>
