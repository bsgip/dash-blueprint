...
- `fixedToTop` (Bool; optional): Whether this navbar should be fixed to the top of the viewport (using CSS position: fixed).
"""
function navbar(; kwargs...)
        available_props = Symbol[:children, :id, :className, :fixedToTop]
        wild_props = Symbol[]
        return Component("navbar", "Navbar", "dash_blueprint", available_props, wild_props; kwargs...)
end
