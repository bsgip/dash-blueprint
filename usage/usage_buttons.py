        ...
        dash_blueprint.Menu(
            children=[
                ...
                dash_blueprint.MenuItem(
                    text="Menu Item 2", href="something", icon="history"
                ),
                dash_blueprint.MenuItem(
                    text="Submenu item",
                    children=[
...
@app.callback(Output("debounce-event", "children"), [Input("resize-watcher", "size")])
def display_page(size):
    print(size)
    return html.Div([html.H3("Size is {}".format(size))])


@app.callback(Output("current-href", "children"), [Input("url", "pathname")])
def display_page(pathname):
    return html.Div([html.H3("You are on page {}".format(pathname))])


@app.callback(Output("my-toaster", "toasts"), [Input("button", "n_clicks")])
def button_clicked(n_clicks):
    print("button clicked! - {}".format(n_clicks))
    messages = [
        {
            "message": "button clicked for the {}th time".format(n_clicks),
    ...
    if n_clicks is not None and n_clicks > 5 and n_clicks < 10:
        messages.append(
            {
                "message": "bonus message for clicking so many times!",
                "intent": "success",
            }
        )
...
@app.callback(Output("second-toaster", "toasts"), [Input("button-2", "n_clicks")])
def button_clicked(n_clicks):
    print("button clicked! - {}".format(n_clicks))
    messages = [
        {
            "message": "2nd button clicked for the {}th time".format(n_clicks),
...
@app.callback(Output("third-toaster", "toasts"), [Input("button-2", "n_clicks")])
def button_clicked(n_clicks):
    messages = [
        {
            "message": "but another message shows up here",
...
@app.callback(Output("output-2", "children"), [Input("popover-button", "n_clicks")])
def button_clicked(n_clicks):
